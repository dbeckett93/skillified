...
class AddEventTests(TestCase):
    ...
    - `test_page_visible_to_mentor`: Ensures that the add event page is only visible to mentor users.
    - `test_page_not_visible_to_non_mentor`: Ensures that the add event page is not visible to non-mentor users.
    - `test_add_event`: Ensures that a mentor user can add a new event with a title, overview, and date/time.
    - `test_add_event_title_required`: Ensures that the event title is a required field.
    - `test_add_event_overview_required`: Ensures that the event overview is a required field.
    - `test_add_event_date_time_required`: Ensures that the event date/time is a required field.
    - `test_add_event_invalid_date_time_format`: Ensures that the event date/time format is validated.
    """
    ...
    def test_page_not_visible_to_non_mentor(self):
        ...
        response = self.client.get(reverse('add_event', args=[self.skill.id]))
        self.assertEqual(response.status_code, 302)  # Redirect to skill detail page
    ...
    def test_add_event_invalid_date_time_format(self):
        ...
        self.assertTrue(form.errors)
        self.assertIn('date_time', form.errors)
        self.assertEqual(form.errors['date_time'], ['Enter a valid date/time.'])