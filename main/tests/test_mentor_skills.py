...
class MentorSkillsTests(TestCase):
    ...
    - `test_add_new_skill_button_not_visible_for_non_mentors`: Ensures that the "Add New Skill" button is not visible to users who are not mentors.
    - `test_add_new_skill_button_visible_for_mentors`: Ensures that the "Add New Skill" button is visible to users who are mentors.
    - `test_search_skills_by_description`: Tests the search functionality by querying skills based on their description.
    - `test_search_skills_by_title`: Tests the search functionality by querying skills based on their title.
    - `test_search_skills_empty_query`: Tests the search functionality with an empty query string.
    - `test_search_skills_no_results`: Tests the search functionality with a query string that yields no results.
    Setup:
    - Creates a client for making HTTP requests.
    - Creates a mentor user and a non-mentor user.
    - Sets up profiles for both users, marking one as a mentor and the other as a non-mentor.
    """
